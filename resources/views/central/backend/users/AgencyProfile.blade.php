	...
	<div class="col-xl-4 col-lg-4 col-md-12">
		<div class="card box-widget widget-user">
			...
			<div class="card-body text-center">
				<div class="pro-user">
					<h4 class="pro-user-username text-dark mb-1 font-weight-bold">{{$AgencyProfile->agency_name}}</h4>
					<h6 class="pro-user-desc text-muted">{{$AgencyProfile->tenant->domains[0]->domain}}</h6>
					
					@switch($AgencyProfile->is_approved)
					@case(0)
					<a href="{{route('admin.agency.approve', ['tenantId' => $AgencyProfile->tenant->id,'status' => 1])}}" class="btn btn-success btn-sm mt-3">Approve</a>
					@break
					@case(1)
					<a href="{{route('admin.agency.approve', ['tenantId' => $AgencyProfile->tenant->id,'status' => 0])}}" class="btn btn-danger btn-sm mt-3">Decline</a>
					@break
					@default
					<a href="{{route('admin.agency.approve', ['tenantId' => $AgencyProfile->tenant->id,'status' => 1])}}" class="btn btn-success btn-sm mt-3">Approve</a>
					<a href="{{route('admin.agency.approve', ['tenantId' => $AgencyProfile->tenant->id,'status' => 0])}}" class="btn btn-danger btn-sm mt-3">Decline</a>
